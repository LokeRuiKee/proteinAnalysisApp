...
def fetch_ppi_network(uniprot_id):
    ...
    params = {
        ...
        'limit': 50,  # Limit interactions to 50
        'caller_identity': 'your_email@example.com',  # Your email (required by the API)
        'network_flavor': 'confidence',
        'required_score': 400  # Minimum interaction score
    }
    
    # Make the API request
    response = requests.post(f'{url}/{method}', data=params)
    ...
    if response.status_code == 200:
        ...
        # Extract and display the interactions
        interactions = data.get('interactions')
        if interactions:
            print(f'Protein-Protein Interactions for {uniprot_id}:')
            for interaction in interactions:
                print(interaction)
        else:
            print('No interactions found.')
    else:
        print('Failed to fetch data from STRING DB.')
